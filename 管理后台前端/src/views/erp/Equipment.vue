  ...
  <div class="equipment-management">
    <el-card>
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <el-form-item label="设备类型">
          <el-select v-model="searchForm.equipmentType" placeholder="请选择" clearable style="width: 120px">
      ...
      >
        ...
        <el-table-column prop="capacityPerHour" label="每小时产能" width="100">
          <template #default="{ row }">
            {{ row.capacityPerHour }} {{ row.capacityUnit }}
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="每小时产能">
              <el-input-number v-model="form.capacityPerHour" :precision="2" :min="0" placeholder="请输入每小时产能" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="产能单位">
              <el-input v-model="form.capacityUnit" placeholder="请输入产能单位" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
...
const handleStatus = (row: any, status: number) => {
  ElMessageBox.confirm(`确定要将设备标记为"${getStatusName(status)}"吗？`, '提示', {
...
const handleBatchStatus = (status: number) => {
  ElMessageBox.confirm(`确定要将选中的 ${selectedIds.value.length} 个设备标记为"${getStatusName(status)}"吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await updateEquipmentStatusBatch(selectedIds.value, status)
      ElMessage.success('操作成功')
      fetchData()
    } catch (error) {
      console.error(error)
...
const handleBatchDelete = () => {
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 个设备吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      await deleteEquipmentBatch(selectedIds.value)
      ElMessage.success('删除成功')
      fetchData()
    } catch (error) {
      console.error(error)

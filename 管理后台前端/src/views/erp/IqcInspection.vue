<template>
  <div class="iqc-inspection-management">
    <el-card>
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <el-form-item label="采购订单号">
          <el-input v-model="searchForm.purchaseOrderNo" placeholder="请输入采购订单号" clearable />
        </el-form-item>
      ...
      <el-table :data="tableData" border stripe v-loading="loading">
        ...
        <el-table-column prop="purchaseOrderNo" label="采购订单号" width="140" />
        ...
        <el-table-column prop="receivedQuantity" label="来料数量" width="100" />
        <el-table-column prop="sampleQuantity" label="抽样数量" width="100" />
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item label="来料数量">
              <el-input-number v-model="form.receivedQuantity" :min="0" :precision="4" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="抽样数量">
              <el-input-number v-model="form.sampleQuantity" :min="0" :precision="4" style="width: 100%" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
...
import {
  getIqcInspectionList,
  createIqcInspection,
  updateIqcInspection,
  deleteIqcInspection,
  submitIqcInspection,
  completeIqcInspection,
  disposalIqcInspection
} from '@/api/iqcInspection'
...
const fetchData = async () => {
  ...
  try {
    ...
    const res = await getIqcInspectionList(params)
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('确定要删除该检验单吗？', '提示', {
    ...
    try {
      await deleteIqcInspection(row.id)
...
const handleSubmit = (row: any) => {
  ElMessageBox.confirm('确定要提交检验吗？提交后将进入待检验状态。', '提示', {
    ...
    try {
      await submitIqcInspection(row.id)
...
const handleCompleteSubmit = async () => {
  ...
  await completeFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        await completeIqcInspection(completeForm.id!, {
...
const handleDisposalSubmit = async () => {
  ...
  await disposalFormRef.value.validate(async (valid) => {
    if (valid) {
      submitLoading.value = true
      try {
        await disposalIqcInspection(disposalForm.id!, {
...
const handleSubmitForm = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        if (isEdit.value) {
          await updateIqcInspection(form.id!, form)
          ElMessage.success('更新成功')
        } else {
          await createIqcInspection(form)
